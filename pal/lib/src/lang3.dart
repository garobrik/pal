...
sealed class Expr {
  ...
  String toString() => switch (this) {
        ...
        FnDef(:var argID, :var argType, :var body) => 'FnDef($argID)($argType)($body)',
  ...
  Set<ID> get freeVars => switch (this) {
        Var(:var id) => {id},
        FnDef(:var argID, :var body) => body.freeVars.difference({argID}),
  ...
  Expr substExpr(ID from, Expr to) => switch (this) {
        ...
        FnDef(:var argID, :var argType, :var body) => FnDef(
            argID,
...
Result<(TypeCtx, Expr, Expr)> check(TypeCtx ctx, Expr? expectedType, Expr expr) {
  ...
  return Success((ctx, actualType, reduce(ctx, redex)));
}
